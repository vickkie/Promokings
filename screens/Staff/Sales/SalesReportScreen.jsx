...
const SalesReportScreen = () => {
  ...
  const { responseData, postData, isLoading } = usePost2("orders/salesReport");
  ...
  const generateSalesForecastHTML = () => {
    if (!data || !data.data || !data.grandTotals) {
      //   Alert.alert("Missing data", "Sales data is not available yet here.");
      console.log("me", data);
      return;
    }
    ...
    const renderWeekRow = (week) => {
      const dayCells = days
        .map((day) => {
          ...
          return `
      <td style="border: 1px solid #ccc; text-align: center;">${formatCurrency(d.pricePerUnit)}</td>
    `;
        })
        .join("");

      const unitsCells = days
        .map((day) => {
          const d = week.days[day];
          return `
      <td style="border: 1px solid #ccc; text-align: center;">${d.unitsSold}</td>
    `;
        })
        .join("");
    ...
    `;
      ...
      return `
    <tr>
      <td rowspan="3" style="border: 1px solid #ccc; text-align: center; background-color: #f0f0f0; font-weight: bold; vertical-align: middle;">${new Date(
        week.weekStartDate
      ).toLocaleDateString()}</td>
      <td style="border: 1px solid #ccc; background-color: #f8f9fa; font-weight: bold; text-align: left; padding-left: 10px;">PRICE PER UNIT</td>
      ${dayCells}
      <td style="border: 1px solid #ccc; text-align: center; background-color: #e3f2fd; font-weight: bold;" rowspan="3">${
        week.totalUnits
      }</td>
    </tr>
    <tr>
      <td style="border: 1px solid #ccc; background-color: #f8f9fa; font-weight: bold; text-align: left; padding-left: 10px;">UNITS SOLD</td>
      ${unitsCells}
    </tr>
    <tr>
  ...
  `;
    ...
    const footerUnits = days
      .map(
        (day) => `
  <td style="border: 1px solid #ccc; text-align: center; background-color: #f0f0f0; font-weight: bold;">${grandTotals.unitsSoldByDay[day]}</td>
`
      )
      .join("");
...
<html>
  ...
  <body>
    ...
    <div class="section">
      ...
      <div class="header">
        <div class="title">PROMOKINGS SALES REPORT</div>
        <div class="subtitle">* Generated by sales department.</div>
      </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td colspan="2" style="border: 1px solid #ccc; background-color: #f8f9fa; font-weight: bold; text-align: left; padding-left: 10px;">UNITS SOLD</td>
            ${footerUnits}
            <td style="border: 1px solid #ccc; text-align: center;" class="grand-totals">${
              grandTotals.grandUnitsSold
            }</td>
          </tr>
          <tr>
      ...
      <View style={styles.container}>
        <View style={styles.wrapper}>
          <View style={styles.upperRow}>
            ...
            <Text style={styles.topheading}>Sales Report</Text>
          </View>
          <ScrollView>
            <View style={styles.lowerRow}>
              <View style={{ padding: 16, gap: 16 }}>
                <Text style={{ fontSize: 18, fontWeight: "bold" }}>📊 Sales Report Filter</Text>

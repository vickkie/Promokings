...
export async function registerForPushNotificationsAsync(userId) {
  ...
  sendLocalNotification("Notification Check", "Checking permissions...");
  ...
  if (status !== "granted") {
    console.log("Failed to get push token for push notification");
    sendLocalNotification("Permission Denied", "Push notifications disabled.");
    return;
  }

  console.log("Notification permission granted:", status);
  sendLocalNotification("Permission Granted", "Retrieving push token...");
  ...
  if (!projectId) {
    console.error("Missing EAS Project ID! Push notifications may fail.");
    sendLocalNotification("Error", "Missing EAS Project ID!");
    return;
  }

  const token = (await Notifications.getExpoPushTokenAsync({ projectId })).data;
  console.log("Expo Push Token:", token);
  sendLocalNotification("Push Token Received", "Sending to backend...");
  ...
  try {
    ...
    console.log("Push token updated successfully", response.data);
    sendLocalNotification("Success", "Push token updated.");
  } catch (err) {
    console.error("Error updating push token:", err);
    sendLocalNotification("Error", "Failed to update push token.");
  }
}
...
export function startPermissionCheck(userId) {
  const interval = setInterval(async () => {
    console.log("Checking notification permission...");
    sendLocalNotification("Permission Check", "Checking notification status...");
    ...
    if (status === "granted") {
      console.log("Permission granted, stopping checks.");
      clearInterval(interval);
      sendLocalNotification("Permission Granted", "Registering for push notifications...");
      registerForPushNotificationsAsync(userId);
    } else if (status === "denied") {
      console.log("Permission denied. User needs to enable manually.");
      sendLocalNotification("Permission Denied", "Enable notifications manually.");
    } else {
      console.log("Permission status is unknown or not determined yet.");
    }
  }, 30000);
